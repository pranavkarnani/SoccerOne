...
csv_file = open('England-Premier-League-fixture-2021-2022.csv', 'wb')
...
df = pd.read_csv('England-Premier-League-fixture-2021-2022.csv', sep=',',index_col=False)
...
team_data_df = pd.read_csv('teams.csv', sep=',',index_col=1)
...
home_strength = dict(zip(team_data_df.name, team_data_df.strength_overall_home))
away_strength = dict(zip(team_data_df.name, team_data_df.strength_overall_away))
...
teams_list = team_data_df['name'].tolist()
...
plot = sns.heatmap(heatmap_df.astype(float), annot=labelsArray,cmap ='RdYlGn_r', fmt = '', vmin=0, vmax=5, 
